...
class HJReadParser: NSObject {
    ...
    /**
     解析本地URL 保存阅读文件,以及书架模型,判断是否已经有加入阅读列表,有则不加入,没有则加入
    ...
    /**
     ...
     - returns: 章节模型数组
     */
    class func separateContent(content:String) ->[HJReadChapterModel] {
        ...
        if !results.isEmpty {
            ...
            (results as NSArray).enumerateObjectsUsingBlock({ (obj, index, stop) in
                ...
                readChapterModel.isDownload = true
                ...
                readChapterModels.append(readChapterModel)
    ...
    class func GetReadFont(fontType:HJReadFont,fontSize:CGFloat) ->UIFont {
        ...
        case .One: // 黑体
            font = UIFont.fontOfNameSize("STHeitiTC-Medium", size: fontSize)
            break
            
        case .Two: // 楷体
            font = UIFont.fontOfNameSize("STKaiti-SC-Regular", size: fontSize)
            break
            
        case .Three: // 宋体
            font = UIFont.fontOfNameSize("STSongti-SC-Regular", size: fontSize)
            break
