...
class HJReadUI: NSObject,HJReadBottomViewDelegate,HJReadLightViewDelegate {
    ...
    func readBottomViewLastChapter(readBottomView: HJReadBottomView) {
        ...
        let isOK = readPageController.readConfigure.GoToReadChapter("\(readPageController.readModel.readRecord.readChapterModel.chapterID.integerValue() - 1)",isInit: false, chapterLookPageClear: true)
        
        if !isOK {MBProgressHUD.showError("没有更多章节了")}
    }
    ...
    func readBottomViewChangeSlider(readBottomView: HJReadBottomView, slider: UISlider) {
        ...
        readPageController.readConfigure.GoToReadChapter("\(Int(slider.value) + 1)", isInit: false, chapterLookPageClear: true)
    }
